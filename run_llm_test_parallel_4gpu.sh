...
# 4-GPU 병렬 LLM 테스트 실행 스크립트 (N회 반복 지원)
# Usage: ./run_llm_test_parallel_4gpu.sh [iterations]
#
# 예시:
#   ./run_llm_test_parallel_4gpu.sh           # 기본값 사용 (1회)
#   ./run_llm_test_parallel_4gpu.sh 5         # 5회 반복
#   ./run_llm_test_parallel_4gpu.sh 10        # 10회 반복
#   ITERATIONS=10 ./run_llm_test_parallel_4gpu.sh  # 환경변수로 10회
...
TEST_IDS_FILE="/app/ksubscribe_share/test/test_ids_green_energy.txt"
...
# 세션별 결과 디렉토리
SESSION_DIR="${BASE_TEST_DIR}/gpu_4way_${SESSION_ID}"
...
echo -e "${CYAN}==========================================${NC}"
echo -e "${CYAN}🚀 4-GPU 병렬 LLM 테스트 (N회 반복 실행)${NC}"
echo -e "${CYAN}==========================================${NC}"
...
do
    ...
    for GPU_ID in 0 1 2 3; do
        ...
        nohup docker exec \
            ...
            geon_python_unified \
            python3 /app/ksubscribe_share/test/test_llm_evaluation.py \
