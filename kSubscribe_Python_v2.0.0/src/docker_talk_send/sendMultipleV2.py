...
def count_matching_words(target: str, word_list: list[str]) -> int:
    return sum(1 for word in word_list if word in target)
...
def recommend_contents_v3(mberId: str, contents: list[ContentsVO]):
    try:
        ...
        for content in contents:
            try:
                ...
                if hasattr(user, "title"):
                    # 본인 키워드 점수 계산 (콘텐츠의 제목에 키워드가 매칭되는지 확인)
                    match_cnt = count_matching_words(content.title, content_predKeywords_keys)
                    keyword_match_score = match_cnt * 1000
...
def send_kakao_multiple(member_list: list[MemberVO], contents: list[ContentsVO], exec_time: datetime):
    ...
    for index, member in enumerate(member_list):
        try:
            ...
            recommand_contents, used_keyword = recommend_contents_v2(member.mberId, contents)
...
def send_telegram_multiple(member_list: list[MemberVO], contents: list[ContentsVO], exec_time: datetime):
    # 모든 멤버를 반복
    for index, member in enumerate(member_list):
        try:
            if not hasattr(member, "mberId"): # 멤버 정보가 없으면 pass
...
def send_email_multiple(member_list: list[MemberVO], contents: list[ContentsVO], exec_time: datetime):
...
docker_collect_logger.info(f"-------------Docker_Talk_Send 시작 ----------------")
...
exec_time = datetime.now()
# 모든 멤버를 반복 - 카카오톡 먼저 보내기
send_kakao_multiple(member_list, today_contents, exec_time)

exec_time = datetime.now()
# 모든 멤버를 반복 - 텔레그램 보내기
send_telegram_multiple(member_list, today_contents, exec_time)

exec_time = datetime.now()
# 모든 멤버를 반복 - 이메일 보내기
send_email_multiple(member_list, today_contents, exec_time)

docker_collect_logger.info(f"-------------Docker_Talk_Send 종료 ----------------")
