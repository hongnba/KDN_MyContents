...
class EmailSendModel:
    ...
    def HTML_BODY_CONTENT(self, content_url: str, content_image_type: str, content_image_source: str,
                          org_image_type: str, org_image_source: str,
                ...
                <div style="max-width: 508px; max-height: 205px; margin: 20px auto; background-color: #ffffff; border-radius: 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); overflow: hidden;">
                    <a href="{content_url}">
                        <div style="display: flex; align-items: center;" border-radius: 10px;>
                            <img src="data:image/{content_image_type};base64,{content_image_source}" alt="News Image" style="width: 200px; height: 175px; margin: 15px 15px;">
                            <div style="flex: 1; display: flex; flex-direction: column; justify-content: space-between; word-break: break-word;">             
                                <div>
                                    <img src="data:image/{org_image_type};base64,{org_image_source}" alt="Organization Image" style="height: 20px; width: auto; object-fit: cover;">
                                </div>
    ...
    def make_html(self, contents: list[ContentsVO]):
        ...
        for content in contents:
            ...
            if hasattr(content, "contentsMeta"): # not 제거할 것.
                if content.contentsMeta != None:
                    if hasattr(content.contentsMeta, "longSummary"):
                        long_summary = content.contentsMeta.longSummary
                    if hasattr(content.contentsMeta, "predKeywords"):
                        predKeywords = list(content.contentsMeta.predKeywords.keys())
            ...
            body_content = self.HTML_BODY_CONTENT(content_url,
                                                  contents_image_type, encoded_contents_image,
                                                  org_img_type, encoded_commcode_image,
                                                  content.contentsOrgName, content.pubDt.strftime("%Y-%m-%d"),
                                                  content.title, long_summary, predKeywords
                                                  )
