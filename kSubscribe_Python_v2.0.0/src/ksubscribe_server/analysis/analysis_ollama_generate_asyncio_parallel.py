...
class AnalysisOllamaGenerateCallAsyncio(AnalysisOllamaBaseAsyncio):
    ...
    def analysis_main(self, content, pred_keyword_list, org_name_list, mycontents_logger:logging.Logger, queueContent:ContentsQueueVO): #-> tuple[bool, ContentsMetaResult]:
        ...
        try: 
            ...
            # 준비: 기관명과 동의어를 프롬프트에 전달할 수 있도록 치환값 생성
            ...
            if related:
                ...
                if isinstance(keywords_verify, list) and len(keywords_verify) > 0:
                    ...
                    # 20260112 모니터링 용: short_summary2와 long_summary 유사도 비교
            ...
            # 20260112 모니터링 용: pred_keywords2, pred_keywords_long도 함께 저장
    ...
    def summary_to_ollamaModel_v2(self,result_summary:GenerateResponse, result_summary_json, contentsMetaResult:ContentsMetaResult, pred_kewords:dict, ai_keywords:list, mycontents_logger:logging.Logger, pred_keywords2:dict=None, pred_keywords_long:dict=None) -> bool:
        ...
        try:
            ...
            # 20260112 모니터링 용: pred_keywords2, pred_keywords_long 저장
            if pred_keywords2 is not None and isinstance(pred_keywords2, dict):
                contentsMetaResult.contentsMeta.pred_keywords2 = self.nomalize_keywords(pred_keywords2)
            if pred_keywords_long is not None and isinstance(pred_keywords_long, dict):
                contentsMetaResult.contentsMeta.pred_keywords_long = self.nomalize_keywords(pred_keywords_long)

            short_summary = result_summary_json["short_summary"]
            if short_summary is not None and isinstance(short_summary,str):
                contentsMetaResult.contentsMeta.shortSummary = short_summary
            
            # 20260112 모니터링 용: short_summary2 추출 및 저장
            short_summary2 = result_summary_json.get("short_summary2")
            if short_summary2 is not None and isinstance(short_summary2, str):
                contentsMetaResult.contentsMeta.short_summary2 = short_summary2
    ...
    def assemble_sentiment_to_ollamaModel_v2(self, queueContent:ContentsQueueVO, orgName:str, result_sentiment_ratio:GenerateResponse, result_sentiment_ratio_json, result_sentiment_reason:GenerateResponse, result_sentiment_reason_json, result_sentiment_keywords:GenerateResponse, result_sentiment_keywords_json, contentsMetaResult:ContentsMetaResult, mycontents_logger:logging.Logger) -> bool:
        ...
        try:
            ...
            # 이전에는 키워드 개수와 비율을 기록했으나 더 이상 저장하지 않음
