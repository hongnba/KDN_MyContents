...
# test 폴더에서 실행되므로 여러 단계의 상위 디렉토리를 추가하여 안전하게 import 가능하도록 함
SCRIPT_DIR = Path(__file__).parent
sys.path.insert(0, str(SCRIPT_DIR))  # test 디렉토리
sys.path.insert(0, str(SCRIPT_DIR.parent))  # ksubscribe_share 디렉토리
sys.path.insert(0, str(SCRIPT_DIR.parent.parent))  # src 디렉토리 (프로젝트 루트)
...
def main():
    ...
    if is_single_date:
        logger.info(f"  조회 날짜: {start_date.strftime('%Y-%m-%d')} (단일 날짜)")
    else:
