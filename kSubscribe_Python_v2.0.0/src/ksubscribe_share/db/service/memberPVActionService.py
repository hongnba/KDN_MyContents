...
class StatsService(BaseQueryService):
    ...
    def _call_ollama_for_analysis(self, stats_data: Dict) -> str:
        ...
        try:
            # Ollama API 설정
            # 이전에 제안된 환경변수 사용 예 (주석 처리됨):
            # import os
            # ollama_base = os.getenv('OLLAMA_BASE_URL')
            # ollama_url = f"{ollama_base.rstrip('/')}/api/generate" if ollama_base else "http://192.168.1.191:11434/api/generate"
            ...
            분석 기간: {stats_data.get('start_date', stats_data.get('startDate', 'N/A'))} ~ {stats_data.get('end_date', stats_data.get('endDate', 'N/A'))}
            ...
            # 모델 하드코딩 (요청대로):
            # 이전에 제안된 환경변수 사용 예 (주석 처리됨):
            # payload_model = os.getenv('OLLAMA_MODEL', 'llama-3-Korean-Bllossom-8B-Q4_K_M')
