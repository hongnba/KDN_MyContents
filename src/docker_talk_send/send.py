...
## HTML UI 업데이트 버전 시작
    ...
    <body>
    	<!-- 시작 -->
    	<!-- width값 조절 -->
    	<div style="width:1000px;font-family:나눔고딕;position:relative;letter-spacing:-1.2px;min-width:700px;">	
    		<!-- 컨텐츠 영역 -->
    		<div style="width:97.5%;">
    		<!-- 뉴스 상단 -->
    		<div style="height:57px;background-image:linear-gradient(90deg,rgba(1,51,177,1),rgba(4,99,196,1));background-position:right;background-repeat:no-repeat;display:table;width:100%;">			
    			<span style="display:table-cell;vertical-align:middle;height:15px;background:url(https://kdcc.kdn.com/anal/images/gudok/logo.png) 15px center no-repeat;color:#fff;font-family:'Arial Black', 'sans-serif';padding-left:65px;font-size:18px;">NEWS CLIPPING</span>
    			<img src="https://kdcc.kdn.com/anal/images/gudok/bg.png" width="357" height="57" style="float:right;">
    		</div>
    		<!-- 뉴스 타이틀 -->
    		<h2 style="color:#004284;font-size:20px;vertical-align:top;position:relative;font-weight:bold; center no-repeat;padding-left:35px; top:10px; height:40px;">한전KDN 콘텐츠 구독 서비스 
    		  <span style="color:#5f5f5f;font-size:16px;position:absolute;right:15px;font-weight:500;float:right;">{today}({get_days(int(today[:4]), int(today[5:7]), int(today[8:]))})</span>
    		</h2>
    		<div style="background:#f7f7f7;border:1px solid #e6e6e6;box-sizing:border-box;padding:20px;">'''
...
# df row를 하나하나 돌면서
    ...
    for idx, row in df.iterrows():
        sheet.append([row['카탈로그명'], row['기관명'], row['분류명'], row['TITLE'], row['URL']])
        sheet.cell(row=sheet.max_row, column=5).style = "Hyperlink"
        # 첫번째 경우. div 새로 만들기 및 초기값 세팅
        print(idx)
        print(row['카탈로그명'], row['기관명'], row['분류명'])
        print(row['TITLE'])
        if first_flag:
            # 카탈로그 div 새로 만들기
            html += f'''
                    <!-- 기업 뉴스1-->
                    <div>
                        <div style="width:95%;padding-right:5px;vertical-align:top;">'''
            # 카탈로그명 세팅(추후 카탈로그 이미지 들어가야 함)
            # html += f'''<h2 style="margin:auto;">{row['카탈로그명']}</h2>'''
            html += f'''<img src="https://kdcc.kdn.com/anal/images/gudok/{row['카탈로그 CI(KDCC)']}.png?v=1" onerror="this.style.display='none'" title="{row['카탈로그명']}" width="{row['카탈로그 CI WIDTH']}" height="{row['카탈로그 CI HEIGHT']}" style="vertical-align:middle; display:inline; margin-right:5px;">'''
            if row['카탈로그 CI SMALL'] == 'Y':
                html += f'''<h2 style="display:inline";>{row['카탈로그명']}</h2>'''
            # 위 div 닫고 분류명 삽입
            html += f'''	
                    </div>
                    <ul style="width:95%;display:inline-block;line-height:28px;margin:0 0 0 25px;padding:0;list-style:none;">
                        <li style="list-style:none;">
                            <p style="font-size: 15px;margin-top: 6px;font-weight:bold;margin:0">□ {row['분류명']}</p>
                        </li>
                        '''
            # 기관 ci 및 내용
            html += f'''
            <li style="width:98%;white-space:nowrap;text-overflow:ellipsis;overflow:hidden;list-style:none;">'''
            # 단일 카탈로그 아닐 경우에만 기관 CI 넣어주기
            if row['하위 기관 수'] != 1:
                html += f'''<img src="https://kdcc.kdn.com/anal/images/gudok/{row['기관 CI(KDCC)']}.png?v=1" onerror="this.style.display='none'" title="{row['기관명']}" width="{row['기관 CI WIDTH']}" height="{row['기관 CI HEIGHT']}" style="vertical-align:middle; display:inline; margin-right:5px;">'''
            if row['기관 CI SMALL'] == 'Y':
                html += f'''<p style="font-size:15px; color:black;font-weight:bold;margin:0;display:inline;">{row['기관명']}</p>'''
            html += f'''<a href="https://mycontents.kdn.com/c/g.do?s={row['SHORT_URL']}&n={row["HASH_SEQ"]}" style="cursor:pointer;font-size:15px;color:black;text-decoration: none;margin-left:10px;display:block;">◾ {row['TITLE']}</a>
            </li>'''

            # 초기값 세팅
            current_catalog = row['카탈로그명']
            current_company = row['기관명']
            current_category = row['분류명']

            first_flag = False
            continue
        # 같은 카탈로그이면
        if current_catalog == row['카탈로그명']:
            # 같은 카탈로그 & 같은 카테고리이면
            if current_category == row['분류명']:
                # 같은 카탈로그 & 같은 카테고리 & 같은 기관이면
                # 내용만
                if current_company == row['기관명']:
                    html += f'''
                    <li style="width:98%;white-space:nowrap;text-overflow:ellipsis;overflow:hidden;list-style:none;">
                    <a href="https://mycontents.kdn.com/c/g.do?s={row['SHORT_URL']}&n={row["HASH_SEQ"]}" style="cursor:pointer;font-size:15px;color:black;text-decoration: none;margin-left:10px;display:block;">◾ {row['TITLE']}</a>
                    </li>'''
                # 같은 카탈로그 & 같은 카테고리 & 다른 기관이면
                # ci도 포함
                else:
                    html += f'''
                    <li style="width:98%;white-space:nowrap;text-overflow:ellipsis;overflow:hidden;list-style:none;">'''
                    html += f'''
                    <img src="https://kdcc.kdn.com/anal/images/gudok/{row['기관 CI(KDCC)']}.png?v=1" onerror="this.style.display='none'" title="{row['기관명']}" width="{row['기관 CI WIDTH']}" height="{row['기관 CI HEIGHT']}" style="vertical-align:middle; display:inline; margin-right:5px;">
                    '''
                    if row['기관 CI SMALL'] == 'Y':
                        html += f'''<p style="font-size:15px; color:black;font-weight:bold;margin:0;display:inline;">{row['기관명']}</p>'''
                    html += f'''
        ...
        # 다른 카탈로그이면
        # 카탈로그 div 태그 닫기. div 새로 만들기. 분류명 기관ci 내용 삽입
        else:
            html += '</div>'
            # html += f'''<div><div style=;">
            html += f'''
                        <div style="width:95%;padding-right:5px;vertical-align:top;margin-top:10px">'''


            # 카탈로그명 세팅(추후 카탈로그 이미지 들어가야 함)
            # html += f'''<h2 style="margin:auto;">{row['카탈로그명']}</h2></div>'''
            html += f'''<img src="https://kdcc.kdn.com/anal/images/gudok/{row['카탈로그 CI(KDCC)']}.png?v=1" onerror="this.style.display='none'" title="{row['카탈로그명']}" width="{row['카탈로그 CI WIDTH']}" height="{row['카탈로그 CI HEIGHT']}" style="vertical-align:middle; display:inline; margin-right:5px;">'''
            if row['카탈로그 CI SMALL'] == 'Y':
                html += f'''<h2 style="display:inline;">{row['카탈로그명']}</h2>'''
    ...
    html += '''
        ...
        <div style="height:15px;background-image:linear-gradient(90deg,rgba(1,51,177,1),rgba(4,99,196,1));background-position:right;background-repeat:no-repeat;display:table;width:100%;position:relative;">			
            
        </div>
        <!-- 맨 오른쪽 막대 배경 -->
        <div style="height:100%;width:25px;position:absolute;right:0;top:0;left:978px;background:linear-gradient(180deg,rgba(1,51,177,1),rgba(4,99,196,1));"></div>
        </div>
        <!-- 끝 -->
        </body>
        </html>'''
...
# # HTML UI 업데이트 버전 끝
    ...
    #html += '<img src="https://kdcc.kdn.com/anal/mail/kisa.jpeg" style="background:none" alt="KISA">'
    # wb.save(f'C:\\Users\\최재혁\\Desktop\\monitoring_{today}_{recipient}.xlsx')
    # wb.save(f'/home/ubuntu/excel/monitoring_{today}_{recipient}.xlsx')
    # 메일 보내기
    # 세션생성, 로그인
    s = smtplib.SMTP('smtp.gmail.com', 587)
    s.ehlo()
    s.starttls()
    s.login('kepcokdnkcs@gmail.com', 'slwcwvmbrihowpie')
    ...
    # recipient = ['hh3990@naver.com', 'jaehyuk_choi2@kdn.com', 'true.962348@kdn.com', 'inhong_min77@kdn.com', 'minsang_kim88@kdn.com', 'foreverwithu.2t@kdn.com', 'rnjsgh_7792@kdn.com', 'selee.215@kdn.com', 'gusthd_0448@kdn.com', 'jongha_4396@kdn.com', 'hj_kim1025@kdn.com', 'wwwww@kospo.co.kr', 'jongseok_21@kdn.com', 'bskim.2049@kdn.com']
    # recipient = ['hh44683990@gmail.com']
    # msg['To'] = 'hh3990@naver.com, true.962348@kdn.com, inhong_min77@kdn.com, minsang_kim88@kdn.com, foreverwithu.2t@kdn.com, rnjsgh_7792@kdn.com, selee.215@kdn.com, gusthd_0448@kdn.com, jongha_4396@kdn.com, jaehyuk_choi2@kdn.com, hj_kim1025@kdn.com, wwwww@kospo.co.kr, jongseok_21@kdn.com, bskim.2049@kdn.com'
    # msg['To'] = 'jaehyuk_choi2@kdn.com, hh3990@naver.com'
    msg['NonTo'] = 'hh3990@naver.com, hh44683990@gmail.com'
    # msg['NonTo'] = 'jaehyuk_choi2@kdn.com, hh3990@naver.com'
     
    # NEW 파일첨부
    # with open(f'C:\\Users\\최재혁\\Desktop\\monitoring_{today}_{recipient}.xlsx', "rb") as attachment:
    #     part = MIMEBase('application', 'octet-stream')
    #     part.set_payload(attachment.read())
    # encoders.encode_base64(part)
    # part.add_header('Content-Disposition', "attachment; filename= " + f'({today}) KDN NEW BUSINESS.xlsx')
    # msg.attach(part)
