...
class WebLoad:
    ...
    async def WebLoad (self, url):

        # LoadTimeStart = time.time()
        html =  await self.Load(url)
        # LoadTimeEnd = time.time()
        
        # TransfimeStart = time.time()
        doc_transform =  await self.Transform(html)
        # TransfimeEmd = time.time()
        
        # SpiltTimeStart = time.time()
        splits =  await self.Split(doc_transform)
    ...
    async def WebTotalLoad (self, url):

        # LoadTimeStart = time.time()
        html =  await self.Load(url)
        # LoadTimeEnd = time.time()
        
        html_total = await self.GetHTML(url)
        
        title = await self.GetTitleParsing(html_total)
        img = await self.GetImgParsing(html_total)
    ...
    async def GetTitleParsing(self, html, tags_to_extract=["h2"]):
    ...
    async def GetImgParsing(self, html, tags_to_extract=["article"]):
        soup = BeautifulSoup(html, 'html.parser')
        extracted_data = []
