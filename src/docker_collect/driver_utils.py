...
def get_driver():
    ...
    for cmd in ["chromium", "chromium-browser", "google-chrome"]:
        try:
            chrome_version = subprocess.check_output([cmd, "--version"]).decode().strip().split()[1].split(".")[0]
            docker_collect_logger.info(f"Chrome 버전 감지 성공: {cmd} -> {chrome_version}")
            break
        except (subprocess.CalledProcessError, FileNotFoundError, IndexError) as e:
            docker_collect_logger.debug(f"Chrome 버전 감지 실패 ({cmd}): {e}")
            continue

    # ChromeDriverManager : 크롬 브라우저에 맞는 크롬 드라이버 설치. (버전 지정 가능)
    try:
        if chrome_version:
            chrome_path = ChromeDriverManager(driver_version=chrome_version).install()
            docker_collect_logger.info(f"ChromeDriver 설치 완료 (버전 지정: {chrome_version}): {chrome_path}")
        else:
            chrome_path = ChromeDriverManager().install()
            docker_collect_logger.info(f"ChromeDriver 설치 완료 (자동 감지): {chrome_path}")
    except Exception as e:
        docker_collect_logger.error(f"ChromeDriver 설치 중 오류 발생: {e}")
        raise
    ...
    docker_collect_logger.info(f"WebDriver 생성 완료")
