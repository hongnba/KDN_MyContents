...
class DockerCollectMain:
    ...
    def distribute(self):
        ...
        try:    
            ...
            driver = None
            try:
                self.docker_collect_logger.info("Chrome 드라이버 초기화 시작...")
                driver = get_driver()
                self.docker_collect_logger.info("Chrome 드라이버 초기화 완료")
            except Exception as e:
                self.docker_collect_logger.error(f"Chrome 드라이버 초기화 실패: {e}", exc_info=True)
                self.docker_collect_logger.info("--------------Docker_Collect 종료 (드라이버 초기화 실패)--------------")
                return
            ...
            if driver:
                try:
                    driver.quit()
                    self.docker_collect_logger.info("Chrome 드라이버 종료 완료")
                except Exception as e:
                    self.docker_collect_logger.error(f"Chrome 드라이버 종료 중 오류: {e}")
            ...
            self.docker_collect_logger.error(f"Docker_Collect 전체 프로세스 오류: {e}", exc_info=True)
            self.docker_collect_logger.info("--------------Docker_Collect 종료 (예외 발생)--------------")
            if driver:
                try:
                    driver.quit()
                except:
